...
import { TrophyIcon, CloseIcon, GemIcon, CoinIcon, BoltIcon } from './icons';
    ...
    <>
      <div className="flex flex-col items-center gap-1">
        <button
          ...
          className="btn-bar-icon relative p-2.5 bg-amber-500 text-white rounded-full border-2 border-amber-600 border-b-amber-700 hover:bg-amber-400 active:scale-95"
        >
          <TrophyIcon size={18} />
          {unlocked > 0 && (
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            >
              <div className="panel-header-game p-5 flex justify-between items-center bg-amber-500 text-white">
                <div>
                              ...
                              <div className="flex items-center gap-1 text-xs font-bold flex-shrink-0">
                                {ach.reward.type === 'gems' && (
                                  <span className={isUnlocked ? 'text-purple-500' : 'text-slate-400'}>
                                    <GemIcon size={12} className="inline mr-0.5" />{ach.reward.value}
                                  </span>
                                )}
                                {ach.reward.type === 'money_mult' && (
                                  <span className={isUnlocked ? 'text-emerald-500' : 'text-slate-400'}>
                                    <CoinIcon size={12} className="inline" />+{Math.round(ach.reward.value * 100)}%
                                  </span>
                                )}
                                {ach.reward.type === 'speed_mult' && (
                                  <span className={isUnlocked ? 'text-blue-500' : 'text-slate-400'}>
                                    <BoltIcon size={12} className="inline" />+{Math.round(ach.reward.value * 100)}%
                                  </span>
                                )}
                              </div>
                            </div>
